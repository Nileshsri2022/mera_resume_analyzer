...
class ResumeApp:
    ...
    def get_dynamic_course_recommendations(self, analysis_id=None, role_name=None):
        """Get dynamic course recommendations - AI-generated first, then fallback to static"""
        courses = []

        # First, try to get AI-generated course recommendations
        if analysis_id:
            try:
                ai_courses = get_ai_course_recommendations(analysis_id)
                if ai_courses:
                    print(f"AI-generated courses found for analysis_id {analysis_id}:")
                    ...
                    print(f"No AI-generated courses found for analysis_id {analysis_id}, falling back to static courses")
            except Exception as e:
                print(f"Error getting AI course recommendations: {str(e)}")

        # If no AI courses or analysis_id not provided, fall back to static courses
        if not courses and role_name:
            try:
                static_courses = get_courses_for_role(role_name)
                if not static_courses:
                    # Try to get courses by category
                    from config.courses import get_category_for_role
                    category = get_category_for_role(role_name)
                    static_courses = COURSES_BY_CATEGORY.get(category, {}).get(role_name, [])
                ...
                print(f"Error getting static course recommendations: {str(e)}")

        return courses

    def get_contextual_videos(self, analysis_results=None, selected_role=None, analysis_id=None):
        """Get contextual video recommendations based on resume analysis - AI first, then fallback"""
        ...
        # First, try to get AI-generated video recommendations
        if analysis_id:
            try:
                ai_resume_videos = get_ai_video_recommendations(analysis_id)
                if ai_resume_videos:
                    print(f"AI-generated video recommendations found for analysis_id {analysis_id}: {len(ai_resume_videos)} videos")
                    # Separate resume and interview videos based on content and add AI flag
                    for video in ai_resume_videos:
                        # Add AI flag and default values for missing fields
                        video['is_ai_generated'] = True
                        video['why_recommended'] = video.get('why_recommended', 'AI-recommended video for your specific needs')
                        video['duration'] = video.get('duration', 'Varies')
                        video['difficulty'] = video.get('difficulty', 'Intermediate')

                        if any(keyword in video.get('description', '').lower() or keyword in video.get('title', '').lower()
                               for keyword in ['resume', 'cv', 'cover letter', 'formatting', 'ats', 'keyword']):
                            contextual_videos['resume_videos'].append(video)
                        else:
                            contextual_videos['interview_videos'].append(video)

                    # If we have AI recommendations, return them (limit to 4 each)
                    contextual_videos['resume_videos'] = contextual_videos['resume_videos'][:4]
                    contextual_videos['interview_videos'] = contextual_videos['interview_videos'][:4]

                    # If we have at least some AI recommendations, return them
                    if contextual_videos['resume_videos'] or contextual_videos['interview_videos']:
                        return contextual_videos
                else:
                    print(f"No AI-generated video recommendations found for analysis_id {analysis_id}, falling back to keyword-based")
            except Exception as e:
                print(f"Error getting AI video recommendations: {str(e)}")

        # Fallback to keyword-based recommendations
        # If no analysis results, return general recommendations
        if not analysis_results:
            # Return 2 videos from each category as general recommendations
            for category, videos in RESUME_VIDEOS.items():
                contextual_videos['resume_videos'].extend(videos[:2])

            for category, videos in INTERVIEW_VIDEOS.items():
                contextual_videos['interview_videos'].extend(videos[:2])
            return contextual_videos

        # Extract improvement areas from analysis results
        improvement_areas = self._identify_improvement_areas(analysis_results)

        # Get resume videos based on improvement areas
        resume_videos = self._match_videos_to_improvements(
            RESUME_VIDEOS, improvement_areas, max_videos=4
        )
        # Mark static videos as not AI-generated
        for video in resume_videos:
            video['is_ai_generated'] = False
        contextual_videos['resume_videos'] = resume_videos

        # Get interview videos based on role and improvement areas
        interview_videos = self._match_videos_to_improvements(
            INTERVIEW_VIDEOS, improvement_areas, max_videos=4
        )
        # Mark static videos as not AI-generated
        for video in interview_videos:
            video['is_ai_generated'] = False
        contextual_videos['interview_videos'] = interview_videos

        return contextual_videos

    def _identify_improvement_areas(self, analysis_results):
        """Identify improvement areas from analysis results"""
        improvement_areas = []

        # Extract from analysis text
        analysis_text = analysis_results.get('full_response', '').lower()

        # Check for specific improvement areas
        if 'ats' in analysis_text or 'tracking system' in analysis_text:
            improvement_areas.append('ats_optimization')

        if 'keyword' in analysis_text or 'keywords' in analysis_text:
            improvement_areas.append('keyword_optimization')

        if 'format' in analysis_text or 'formatting' in analysis_text:
            improvement_areas.append('format_optimization')

        if 'content' in analysis_text or 'writing' in analysis_text:
            improvement_areas.append('content_optimization')

        if 'section' in analysis_text or 'structure' in analysis_text:
            improvement_areas.append('section_optimization')

        if 'technical' in analysis_text or 'coding' in analysis_text:
            improvement_areas.append('technical_skills')

        if 'communication' in analysis_text or 'soft skills' in analysis_text:
            improvement_areas.append('communication_skills')

        if 'interview' in analysis_text:
            improvement_areas.append('interview_technique')

        # Check for specific weaknesses mentioned
        weaknesses = analysis_results.get('weaknesses', [])
        for weakness in weaknesses:
            weakness_lower = weakness.lower()
            if 'experience' in weakness_lower:
                improvement_areas.append('experience_description')
            if 'skills' in weakness_lower:
                improvement_areas.append('skills_presentation')
            if 'summary' in weakness_lower:
                improvement_areas.append('summary_writing')

        # If no specific areas identified, add general ones
        if not improvement_areas:
            improvement_areas = ['content_optimization', 'format_optimization', 'interview_technique']

        return list(set(improvement_areas))  # Remove duplicates

    def _match_videos_to_improvements(self, video_database, improvement_areas, max_videos=4):
        """Match videos to improvement areas"""
        matched_videos = []
        used_videos = set()  # Track used video URLs to avoid duplicates

        # Priority matching: videos that match multiple improvement areas first
        for improvement_area in improvement_areas:
            for category, videos in video_database.items():
                for video in videos:
                    ...
                    # Check if video addresses this improvement area
                    if improvement_area in video.get('improvement_areas', []):
                        matched_videos.append({
                            ...
                            'why_recommended': self._generate_video_rationale(video, improvement_area),
                            'category': category
                        })
        ...
        # If we don't have enough matched videos, add some general ones
        if len(matched_videos) < max_videos:
            for category, videos in video_database.items():
                for video in videos:
                    if video['url'] not in used_videos:
                        matched_videos.append({
                            'title': video['title'],
                            'url': video['url'],
                            'duration': video['duration'],
                            'difficulty': video['difficulty'],
                            'description': video['description'],
                            'why_recommended': "General recommendation to improve your resume and interview skills",
                            'category': category
                        })
                        used_videos.add(video['url'])
    ...
    def _generate_video_rationale(self, video, improvement_area):
        """Generate a rationale for why this video is recommended"""
        rationales = {
            'ats_optimization': "This video will help you optimize your resume for Applicant Tracking Systems",
            'keyword_optimization': "Learn how to incorporate relevant keywords into your resume effectively",
            'format_optimization': "Discover professional formatting techniques to make your resume stand out",
            'content_optimization': "Improve your resume content and writing to better showcase your qualifications",
            'section_optimization': "Learn how to structure and organize your resume sections effectively",
            'technical_skills': "Enhance your technical skills presentation for better job prospects",
            'communication_skills': "Develop better communication skills for interviews and professional interactions",
            'interview_technique': "Master interview techniques and best practices",
            'experience_description': "Learn how to effectively describe your work experience",
            'skills_presentation': "Improve how you present your skills on your resume",
            'summary_writing': "Create compelling professional summaries that grab attention"
        }

        return rationales.get(improvement_area, "This video provides valuable insights for career development")
    ...
    st.session_state.form_data['skills_categories']['tools']),
        ...
        if st.button("Generate Resume 📄", type="primary"):
            ...
            print(f"Debug: About to log email input value")
    ...
    "</ul></div>", unsafe_allow_html=True)
                    ...
                    st.markdown("""
                        ...
                        # Get dynamic course recommendations (AI-generated first, then static)
                    courses = self.get_dynamic_course_recommendations(
                        analysis_id=resume_id if 'resume_id' in locals() else None,
                    ...
                    if courses:
                        ...
                        for i, course in enumerate(courses[:6]):  # Show top 6 courses
                            with cols[i % 2]:
                                ...
                                ai_badge = ""
                                if course.get('is_ai_generated', False):
                                    ai_badge = '<span style="background: linear-gradient(90deg, #4CAF50, #45a049); color: white; padding: 2px 6px; border-radius: 10px; font-size: 10px; margin-left: 8px;">AI</span>'

                                course_name = course['name']
                                course_url = course['url'] or "#"
                                course_desc = course.get('description', '')

                                # Create the HTML content using string concatenation to avoid f-string issues
                                course_html = "<div style='background-color: #1e1e1e; padding: 15px; border-radius: 10px; margin: 10px 0; border: 1px solid rgba(76,175,80,0.3);'>"
                                course_html += "<h4 style='margin-bottom: 8px;'>" + course_name + ai_badge + "</h4>"

                                if course_desc:
                                    course_html += '<p style="color: #aaa; font-size: 12px; margin: 8px 0;">' + course_desc + '</p>'

                                course_html += "<a href='" + course_url + "' target='_blank' style='color: #4CAF50; text-decoration: none; font-weight: 500;'>View Course →</a>"
                                course_html += "</div>"

                                st.markdown(course_html, unsafe_allow_html=True)
                    else:
                        st.info("No course recommendations available for this role.")
                    ...
                    # Get contextual video recommendations based on analysis
                    contextual_videos = self.get_contextual_videos(analysis, selected_role)

                    tab1, tab2 = st.tabs(["Resume Tips", "Interview Tips"])

                    with tab1:
                        # Resume Videos - contextual recommendations
                        if contextual_videos['resume_videos']:
                            for video in contextual_videos['resume_videos']:
                                # Check if this is an AI-generated video
                                is_ai_video = video.get('is_ai_generated', False)
                                ai_badge = '<span style="background: linear-gradient(90deg, #4CAF50, #45a049); color: white; padding: 2px 6px; border-radius: 10px; font-size: 10px; margin-left: 8px;">AI</span>' if is_ai_video else ''

                                st.markdown(f"""
                                <div style='background-color: #1e1e1e; padding: 15px; border-radius: 10px; margin: 10px 0; border: 1px solid rgba(76,175,80,0.3);'>
                                    <h4 style='margin-bottom: 8px; color: #4CAF50;'>{video['title']}{ai_badge}</h4>
                                    <p style='color: #aaa; font-size: 12px; margin: 8px 0;'><strong>Duration:</strong> {video['duration']} | <strong>Difficulty:</strong> {video['difficulty']}</p>
                                    <p style='color: #ddd; font-size: 14px; margin: 8px 0;'>{video['description']}</p>
                                    <p style='color: #4CAF50; font-size: 13px; margin: 8px 0; font-style: italic;'><strong>Why recommended:</strong> {video['why_recommended']}</p>
                                </div>
                                """, unsafe_allow_html=True)
                                st.video(video['url'])
                        else:
                            st.info("No specific resume videos recommended for this analysis.")

                    with tab2:
                        # Interview Videos - contextual recommendations
                        if contextual_videos['interview_videos']:
                            for video in contextual_videos['interview_videos']:
                                # Check if this is an AI-generated video
                                is_ai_video = video.get('is_ai_generated', False)
                                ai_badge = '<span style="background: linear-gradient(90deg, #4CAF50, #45a049); color: white; padding: 2px 6px; border-radius: 10px; font-size: 10px; margin-left: 8px;">AI</span>' if is_ai_video else ''
                            ...
                            st.info("No specific interview videos recommended for this analysis.")
        ...
        with analyzer_tabs[1]:
            ...
            selected_custom_model = None
            base_url = None
            api_key = None
            
            if ai_model == "OpenAI Compatible":
                col_api1, col_api2 = st.columns(2)
                with col_api1:
                    base_url = st.text_input("Base URL", placeholder="https://api.openai.com/v1", key="openai_base_url")
                with col_api2:
                        ...
                        try:
                            with st.spinner("Fetching models..."):
                                ...
                                if response.status_code == 200:
                                    ...
                                    st.session_state['fetched_models'] = model_list
                                    st.success(f"Successfully fetched {len(model_list)} models!")
                                else:
                ...
                if 'fetched_models' in st.session_state and st.session_state['fetched_models']:
                    selected_custom_model = st.selectbox("Select Specific Model", st.session_state['fetched_models'], key="custom_model_select")
    ...
    "Upload your resume", type=[
                    ...
                    with st.spinner(f"Analyzing your resume with {ai_model}..."):
                        ...
                        try:
                            ...
                            with st.spinner("🧠 AI is analyzing your resume..."):
                                ...
                                # Analyze the resume
                                if ai_model == "OpenAI Compatible":
                                    if not selected_custom_model or not base_url or not api_key:
                                        st.error("Please select a model and provide Base URL and API Key.")
                                        st.stop()
                                    
                                    selected_model = selected_custom_model
                                    
                                    if use_custom_job_desc and custom_job_description:
                                        analysis_result = analyzer.analyze_resume_with_openai_compatible(
                                            resume_text, base_url, api_key, selected_custom_model, job_description=custom_job_description, job_role=job_role)
                                        st.session_state['used_custom_job_desc'] = True
                                    else:
                                        analysis_result = analyzer.analyze_resume_with_openai_compatible(
                                            resume_text, base_url, api_key, selected_custom_model, job_role=job_role)
                                ...
                                if analysis_result and "error" not in analysis_result:
                                    ...
                                    # Save to database
                                    analysis_id = save_ai_analysis_data(
                                    ...
                                    # Extract and save course recommendations
                                    if analysis_id:
                                        try:
                                            # Extract course recommendations from the analysis text
                                            course_recommendations = self.ai_analyzer.extract_course_recommendations(analysis_result.get("analysis", ""))

                                            if course_recommendations:
                                                # Save course recommendations to database
                                                save_ai_course_recommendations(analysis_id, course_recommendations)
                                                print(f"Saved {len(course_recommendations)} course recommendations for analysis {analysis_id}")
                                        except Exception as course_error:
                                            print(f"Error saving course recommendations: {str(course_error)}")
                                ...
                                if analysis_result and "error" not in analysis_result:
                                    ...
                                    # Store analysis context for tailoring
                                    st.session_state['analysis_context'] = {
                                        ...
                                        'base_url': base_url,
                                        'api_key': api_key,
                                        'selected_custom_model': selected_custom_model,
                                        'use_custom_job_desc': use_custom_job_desc
                                    }
                                    ...
                                    # Extract skills
                                    current_skills = analyzer.extract_skills_from_analysis(full_response)
                                    missing_skills = analyzer.extract_missing_skills_from_analysis(full_response)

                                    if current_skills or missing_skills:
                                        st.markdown("---")
                                        st.subheader("📊 Visual Skills Gap Analysis")
                                        ...
                                        skill_data = {
                                            "Status": ["Matching Skills"] * len(current_skills) + ["Missing Skills"] * len(missing_skills),
                                            "Count": [1] * (len(current_skills) + len(missing_skills))
                                        }
                                        ...
                                        if not df_skills.empty:
                                            ...
                                            with col1:
                                                ...
                                                fig_skills = px.bar(
                                                    ...
                                                    color_discrete_map={"Matching Skills": "#4CAF50", "Missing Skills": "#FF5252"},
                                                    labels={"Count": "Number of Skills"}
                                                )
                                                # Hide legend to save space if redundant
        ...
        # --- Resume Tailoring Section (Outside the main analysis block) ---
        if 'analysis_context' in st.session_state:
            context = st.session_state['analysis_context']
            if context.get('use_custom_job_desc') and context.get('custom_job_description'):
                st.markdown("---")
                st.subheader("✨ AI Resume Tailoring")
                st.info("Based on your provided job description, AI can rewrite your resume to better match the role.")
                
                if st.button("🪄 Tailor Resume to Job", type="primary", key="tailor_resume_btn"):
                    with st.spinner("Writing tailored resume..."):
                        try:
                            analyzer = AIResumeAnalyzer()
                            if context.get('ai_model') == "OpenAI Compatible":
                                tailored_resume = analyzer.tailor_resume_to_job(
                                    context['resume_text'], 
                                    context['custom_job_description'], 
                                    base_url=context.get('base_url'), 
                                    api_key=context.get('api_key'), 
                                    model_name=context.get('selected_custom_model')
                                )
                            else:
                                tailored_resume = analyzer.tailor_resume_to_job(
                                    context['resume_text'], 
                            ...
                            st.error(f"Failed to tailor resume: {str(e)}")
                
                if 'tailored_resume' in st.session_state:
                    st.success("Resume tailored successfully! Download it below.")
